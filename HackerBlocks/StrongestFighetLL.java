...
public class StrongestFighetLL {
	private class Node {
		int data;
		Node next;
	...
	public StrongestFighetLL() {
		this.head = null;
		this.tail = null;
		this.size = 0;
	}

	public StrongestFighetLL(Node head, Node tail, int size) {
		this.head = head;
		this.tail = tail;
		this.size = size;
	}
	...
	public void triplet(StrongestFighetLL n1, StrongestFighetLL n2, int n) {
		triplet(this.head, n1.head, n2.head, n);
	}

	static int count = 0;

	private void triplet(Node n1, Node n2, Node n3, int n) {
		if (n1 == null || n2 == null || n3 == null) {
			return;
		}
		if ((n1.data + n2.data + n3.data) == n && count == 0) {
			count++;
			System.out.println(n1.data + " " + n2.data + " " + n3.data);
			return;
		}
		if (count == 0) {
			triplet(n1.next, n2, n3, n);
			triplet(n1, n2.next, n3, n);
			triplet(n1, n2, n3.next, n);
		}
		return;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		int n = in.nextInt();
		StrongestFighetLL ln = new StrongestFighetLL();
		for (int i = 0; i < n; i++) {
			ln.addLast(in.nextInt());
		}
		int k = in.nextInt();
		ln.StrongestFighter(k);
	}
}
